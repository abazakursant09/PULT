...
- Пер-попыточная таблица аудита с plaintext email+IP удалена (не было читателей); конкретные auth-события в `routers/auth.py` журналируются по идентификатору пользователя без plaintext email/IP/токена (`routers/auth.py:102-105,164,209`). **Это НЕ глобальный инвариант «приложение никогда не пишет ПДн в логи».** SMTP-mailer `services/email.py` (LEGAL-PRELAUNCH-D) больше НЕ логирует адрес получателя, subject или текст исключения: dev/success/error-ветки пишут только event (`email_not_sent`/`email_sent`/`email_send_failed`) и closed-vocabulary category, выведенную по ТИПУ исключения (`_smtp_error_category`, frozen allowlist); тело/токен не логируются; guard `backend/tests/test_mail_log_privacy_guard.py`. Retention/access и место хранения application/security-логов НЕ определены — отдельный operational-блокер #25 (до production).
...
| Security/login logs | недоказанный публичный срок «≤90 дней» удалён из privacy-страницы (LEGAL-PRELAUNCH-E2); фактический destination = stdout/stderr (`backend/main.py:10`, basicConfig без handler); enforced rotation/deletion/max-age отсутствуют; production retention = FUTURE-INFRA (blocker #25) | **промис скорректирован; enforced retention НЕ реализован** — не выдавать за работающий TTL | `frontend/app/privacy/page.tsx`; `backend/main.py:10`; sweep не найден |
