...
- Пер-попыточная таблица аудита с plaintext email+IP удалена (не было читателей); конкретные auth-события в `routers/auth.py` журналируются по идентификатору пользователя без plaintext email/IP/токена (`routers/auth.py:102-105,164,209`). **Это НЕ глобальный инвариант «приложение никогда не пишет email в логи».** SMTP-mailer `services/email.py` логирует адрес получателя (`to=%s`) и subject в dev/success/error-ветках (`:49,53,56`), а при ошибке — текст исключения `str(exc)` (`:56`); тело письма с токеном не логируется (`:45-48`). Устранение/псевдонимизация email в SMTP-логах — блокер до production.
