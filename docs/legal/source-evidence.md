...
| Согласие при регистрации фиксируется на сервере (LEGAL-PRELAUNCH-F2): backend требует явный `consent=true` (StrictBool), отклоняет отсутствие/false до создания пользователя, пишет append-only `consent_records` (server UTC + server `consent_doc_version` + context) в одной транзакции с user; legacy users — evidence отсутствует, backfill НЕ выполняется | `backend/models/consent_record.py`, `backend/routers/auth.py`, `backend/schemas/auth.py`, `frontend/lib/api.ts` | IMPLEMENTED (механизм; юр. достаточность = REQUIRES RUSSIAN COUNSEL; retention → #25) |
...
| Живые страницы всё ещё используют `biznes-pult.ru` / `hello@biznes-pult.ru` (website pult-os.ru не активирован, live-domain swap не выполнялся) | `frontend/app/terms/page.tsx:58,70,86`, охрана `backend/tests/test_domain_migration_prep_guard.py:66` | live pages unchanged |
| Mail Gate PASS: 1 Yandex 360 org, 1 оплачиваемый ящик; support/privacy/security TECHNICALLY ACTIVE — DOCUMENT NOT PUBLISHED — NOT A PUBLIC PRODUCT CONTACT YET; dmarc@ — технический alias | внешняя инфраструктура (проверено Иналом); в коде не hardcoded (`config.py` без pult-os.ru) | infra-only; NOT PUBLICLY ACTIVATED |
| DMARC applied: `v=DMARC1; p=none; rua=mailto:dmarc@pult-os.ru`; quarantine/reject NOT ENABLED; monitoring ≥ 7 days | `docs/dns-runbook-pult-os.md` (applied mail state); registrar zone | p=none monitoring; raise = separate Inal gate |
| application SMTP OFF (`smtp_host` пуст по умолчанию → mailer логирует, не отправляет) | `backend/config.py:55-58,315-316` | OFF |
