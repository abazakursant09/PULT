...
| SMTP-mailer логирует адрес получателя `to=` и subject (dev/success/error), при ошибке — `str(exc)`; тело/токен не логируются | `backend/services/email.py:49,53,56` (и `:45-48`) | GAP до production — псевдонимизировать/убрать email в логах |
