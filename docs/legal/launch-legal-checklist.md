...
|---|---|---|---|---|---|---|---|
| 1 | **Mail Gate**: рабочий ящик на pult-os.ru для обращений субъектов ПДн | Mail Gate PASS: support@ — основной ящик; privacy@/security@ — пользовательские алиасы; dmarc@ — технический alias только для агрегированных отчётов; отправка и получение проверены; пользовательские контакты технически работают, но ещё не активированы публично | DONE | Inal + mail | нет | **да** | да |
...
| 6 | Server-side consent evidence (версия+UTC-время+способ) | механизм реализован **IMPLEMENTED** (LEGAL-PRELAUNCH-F2): backend требует явный `consent=true` (StrictBool), отклоняет отсутствие/false до создания пользователя, пишет append-only `consent_records` (server UTC + server `consent_doc_version` + context registration/recovery) в одной транзакции с user (`backend/models/consent_record.py`, `backend/routers/auth.py`, `backend/schemas/auth.py`); юридическая достаточность = REQUIRES RUSSIAN COUNSEL REVIEW; legacy users — evidence отсутствует, backfill НЕ выполняется (re-consent = отдельное решение Inal+юрист); retention consent evidence → блокер #25 (не решён) | PARTIAL | developer | рекомендуется | **да** | да |
...
| 22 | SMTP / application-log: убрать email получателя, subject и raw exception из логов mailer | recipient/subject/`str(exc)` удалены; mailer логирует только event + closed-vocabulary category по ТИПУ исключения **IMPLEMENTED** (LEGAL-PRELAUNCH-D, `services/email.py`; guard `test_mail_log_privacy_guard.py`); retention/access для application logs — отдельный operational-блокер #25 | PARTIAL | developer | рекомендуется | **да** | да |
