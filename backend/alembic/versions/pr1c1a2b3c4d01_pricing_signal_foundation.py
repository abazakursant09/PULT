"""Pricing/Margin signal data foundation (A3-pre) — pricing_signal
...
(seo/advertising/review/growth/legal): a lifecycle entity keyed by insight_key
`pricing_<problem_type>:<marketplace>:<sku>`, carrying the five PULT doctrine
fields + evidence_hash for reconciliation. Marketplace-agnostic, soft refs.
SQLite-safe (CREATE TABLE + CREATE INDEX) and Postgres-compatible. No binding, no
executor, no payload — schema only.

Revision ID: pr1c1a2b3c4d01
Revises: prom1a2b3c4d01
...
revision: str = "pr1c1a2b3c4d01"
down_revision: Union[str, None] = "prom1a2b3c4d01"
...
def upgrade() -> None:
    ...
    op.create_index("ix_pricing_signal_user_listing", "pricing_signal", ["user_id", "listing_id"])
    op.create_index("ix_pricing_signal_insight", "pricing_signal", ["insight_key"])
    op.create_index("ix_pricing_signal_status", "pricing_signal", ["status"])


def downgrade() -> None:
    op.drop_index("ix_pricing_signal_status", table_name="pricing_signal")
    op.drop_index("ix_pricing_signal_insight", table_name="pricing_signal")
    op.drop_index("ix_pricing_signal_user_listing", table_name="pricing_signal")
    op.drop_table("pricing_signal")
