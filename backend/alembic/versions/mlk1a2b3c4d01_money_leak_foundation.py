"""Money Leak Detection contour data foundation (Phase 3.0) — money_leak_signal + money_leak_audit
...
signal keyed by insight_key `money_leak_<problem_type>:<marketplace>:<sku>` carrying the
...
Revision ID: mlk1a2b3c4d01
Revises: rev1a2b3c4d01
...
revision: str = "mlk1a2b3c4d01"
down_revision: Union[str, None] = "rev1a2b3c4d01"
...
def upgrade() -> None:
    ...
    op.create_index("ix_money_leak_audit_user_listing", "money_leak_audit", ["user_id", "listing_id"])
    op.create_index("ix_money_leak_audit_status", "money_leak_audit", ["status"])
    ...
    op.create_index("ix_money_leak_signal_user_listing", "money_leak_signal", ["user_id", "listing_id"])
    op.create_index("ix_money_leak_signal_insight", "money_leak_signal", ["insight_key"])
    op.create_index("ix_money_leak_signal_audit", "money_leak_signal", ["audit_id"])
    op.create_index("ix_money_leak_signal_status", "money_leak_signal", ["status"])
    op.create_index("ix_money_leak_signal_category", "money_leak_signal", ["category"])


def downgrade() -> None:
    op.drop_index("ix_money_leak_signal_category", table_name="money_leak_signal")
    op.drop_index("ix_money_leak_signal_status", table_name="money_leak_signal")
    op.drop_index("ix_money_leak_signal_audit", table_name="money_leak_signal")
    op.drop_index("ix_money_leak_signal_insight", table_name="money_leak_signal")
    op.drop_index("ix_money_leak_signal_user_listing", table_name="money_leak_signal")
    op.drop_table("money_leak_signal")

    op.drop_index("ix_money_leak_audit_status", table_name="money_leak_audit")
    op.drop_index("ix_money_leak_audit_user_listing", table_name="money_leak_audit")
    op.drop_table("money_leak_audit")
