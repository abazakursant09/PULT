"""
Money Leak Audit (Money Leak Detection contour data foundation, Phase 3.0) — append-only run.

Money Leak Detection looks for WHY money leaks — silent cost-structure drift (commission /
logistics growth, hidden margin erosion). One row per money-leak diagnosis run of a
...
per run. No public score, no fabricated index — only deterministic detected drift.
"""
import uuid
...
class MoneyLeakAudit(Base):
    __tablename__ = "money_leak_audit"
    ...
    total_problems       = Column(Integer, nullable=False, default=0, server_default="0")  # leaks found
    ...
    __table_args__ = (
        Index("ix_money_leak_audit_user_listing", "user_id", "listing_id"),
        Index("ix_money_leak_audit_status", "status"),
    )
