"""
Supply Audit (Supply / Replenishment Diagnosis contour data foundation, Phase 4.0) —
append-only run.

Supply Diagnosis looks for stock-out / supply risk — days-to-out-of-stock and supply_risk
from observed stock + sell-through velocity. One row per supply diagnosis run of a listing.
NO rule logic here — plain table mirror, modelled field-for-field on growth_audit.
Marketplace-agnostic: `marketplace` is provenance/dispatch only. Append-only: written once
per run. No public score, no fabricated index — only deterministic detected risk.
"""
import uuid
...
class SupplyAudit(Base):
    __tablename__ = "supply_audit"
    ...
    source       = Column(String(20), nullable=True)     # finance | catalog | manual
    ...
    total_problems       = Column(Integer, nullable=False, default=0, server_default="0")  # risks found
    ...
    __table_args__ = (
        Index("ix_supply_audit_user_listing", "user_id", "listing_id"),
        Index("ix_supply_audit_status", "status"),
    )
