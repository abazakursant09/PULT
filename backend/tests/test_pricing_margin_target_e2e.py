"""
A4-margin-target E2E — cost-plus pricing loop:

  PricingSignal(pricing_margin_below_target) → promote → bridge → Decision(set_price)
  → preview → apply (set_price @ cost-plus) → measurement open (net_profit) → close
  → EngineEffectObservation → Learning OS.

price = (cogs/unit + logistics/unit) / (1 - commission_rate - PricingRule.target_margin_pct/100)
— observed + seller-defined target. WB + Ozon, isolated. Measured net_profit.
compute_recommendation never called.
"""
import asyncio
import json
import uuid
...
async def _seed(db, uid, *, mp, sku="SKU1", target_margin_pct=25.0):
    phys = str(uuid.uuid4())
    db.add(PhysicalProduct(id=phys, user_id=uid, title="товар", cogs=50.0, cogs_source="manual"))
    pid = str(uuid.uuid4())
    db.add(Product(id=pid, user_id=uid, name="товар", marketplace=mp, sku=sku, price=80.0))
    db.add(ProductListing(physical_product_id=phys, user_id=uid, marketplace=mp, external_id=sku))
    db.add(PricingRule(product_id=pid, min_price=1.0, max_price=9999.0,
    ...
    ikey = f"pricing_margin_below_target:{mp}:{sku}"
    db.add(PricingSignal(user_id=uid, signal_key="pricing_margin_below_target",
           problem_type="margin_below_target", insight_key=ikey, marketplace=mp, sku=sku,
    ...
    db.add(ImportedFinanceRow(import_id=str(uuid.uuid4()), user_id=uid, marketplace=mp,
                              ...
                              logistics=100.0, quantity=10, net_profit=200.0))   # baseline thin
    await db.commit()
...
async def _full_loop(monkeypatch, *, mp):
    ...
    assert BY_SIGNAL_TYPE["pricing_margin_below_target"].safety_class == "manual_approval"
    assert BY_SIGNAL_KEY["pricing_margin_below_target"].default_metric_key == "net_profit"
    ...
    assert p.payload == {"offer_id": "SKU1", "price": COST_PLUS, "old_price": 80.0}
    ...
    assert res.ok and res.status == "success" and calls == [("SKU1", COST_PLUS)]
    ...
    await _fin_after(db, uid, mp=mp, sku=sku, net_profit=900.0)
...
# ── missing target_margin_pct → not applyable, honest payload_not_derivable ──

def test_missing_target_margin_bails_honestly(monkeypatch):
    _explode_recommendation(monkeypatch)
    async def go():
        db = await _engine(); uid = str(uuid.uuid4()); sku = "SKU1"
        await _seed(db, uid, mp="wb", sku=sku, target_margin_pct=None)   # unset
        await _promote(db, uid)
        decision = (await db.execute(select(Decision))).scalars().one()
        p = await build_apply_preview(db, user_id=uid, decision_id=decision.id, marketplace="wb", sku=sku)
        assert p.applyable is False and p.payload is None
        assert p.reason == "payload_not_derivable"
    _run(go())


# ── missing finance → not_evaluated ──────────────────────────────────────────
