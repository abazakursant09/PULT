...
def test_no_advisory_runtime_producer():
    ...
    assert "money_leak" not in keys
    assert "money_leak_detection" not in keys


def test_decision_feed_does_not_reference_money_leak():
    from services.decision_feed.builder import _ENGINES
    tables = {t for (_c, _m, t) in _ENGINES}
    assert "money_leak_signal" not in tables
    contours = {c for (c, _m, _t) in _ENGINES}
    assert "money_leak" not in contours
