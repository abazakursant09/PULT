"""
Review Signal Reconciliation (A6) — give review signals memory + lifecycle.

Instead of creating a fresh signal every audit, reconcile the current audit's rule
outcomes against existing signals for the same (user, listing), keyed by the
canonical insight_key `rev_<problem_type>:<marketplace>:<sku>:<review_id>`. One
signal row per insight_key — a duplicate live signal is structurally impossible,
per review.
...
A NOT_EVALUATED rule NEVER resolves a signal.

Pure DB reconciliation: no API, no Decision bridge, no measurement, no AI, no reply
generation, no marketplace-specific code. Flush-only.
"""
...
def _apply_draft(sig: ReviewSignal, draft, *, evh, audit_id, problem_id, now, status):
    ...
    sig.safety_category = draft.safety_category
    sig.safety_mode = draft.safety_mode
    sig.evidence_hash = evh
    sig.status = status
    sig.updated_at = now


def _new_signal(*, audit_id, user_id, listing_id, marketplace, sku, review_id, draft, evh,
                problem_id, now):
    return ReviewSignal(
        audit_id=audit_id, problem_id=problem_id, review_id=review_id, user_id=user_id,
        listing_id=listing_id, marketplace=marketplace, sku=sku, signal_key=draft.signal_key,
        insight_key=draft.insight_key, problem_type=draft.problem_type,
        safety_category=draft.safety_category, safety_mode=draft.safety_mode,
...
async def reconcile_signals(
    db: AsyncSession, *, user_id: str, listing_id, audit_id: str, marketplace, sku, review_id,
    ...
    rows = (await db.execute(select(ReviewSignal).where(
        ReviewSignal.user_id == user_id, ReviewSignal.listing_id == listing_id))).scalars().all()
    ...
    for e in evaluations:
        ...
        if e.result == RuleResult.TRIGGERED:
            ...
            draft = build_signal(e, marketplace=marketplace, sku=sku, review_id=review_id)
            pid = problem_id_by_type.get(e.problem_type)
            if sig is None:
                new = _new_signal(audit_id=audit_id, user_id=user_id, listing_id=listing_id,
                                  marketplace=marketplace, sku=sku, review_id=review_id,
                                  draft=draft, evh=evh, problem_id=pid, now=now)
