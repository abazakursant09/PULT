"""
SEO Signal Reconciliation (A6) — give signals memory + lifecycle.

Instead of creating a fresh signal every audit, reconcile the current audit's
rule outcomes against the existing signals for the same (user, listing), keyed by
the canonical insight_key. One signal row per insight_key — a duplicate live
...
(problem definitively gone). A NOT_EVALUATED rule (couldn't check) NEVER resolves
a signal — unknown is not "fixed".

Pure DB reconciliation: no API, no Decision bridge, no measurement, no AI, no
marketplace-specific code. Flush-only (caller owns the transaction).
"""
...
RESOLVED = "resolved"
REOPENED = "reopened"
_LIVE = {ACTIVE, REOPENED}   # "open" states — at most one per insight_key
...
def _evidence_hash(evidence: Optional[Mapping[str, object]]) -> str:
    return hashlib.sha256(
        json.dumps(evidence or {}, sort_keys=True, ensure_ascii=False).encode("utf-8")
    ).hexdigest()


def _apply_draft(sig: SeoSignal, draft, *, evidence_hash, audit_id, problem_id, now, status):
    ...
    sig.evidence_hash = evidence_hash
    sig.status = status
    sig.updated_at = now


def _new_signal(*, audit_id, user_id, listing_id, marketplace, sku, draft, evidence_hash,
                problem_id, now) -> SeoSignal:
    return SeoSignal(
        ...
        confidence=draft.confidence, status=ACTIVE, evidence_hash=evidence_hash,
...
async def reconcile_signals(
    ...
    rows = (await db.execute(select(SeoSignal).where(
        SeoSignal.user_id == user_id, SeoSignal.listing_id == listing_id))).scalars().all()
    by_key = {r.insight_key: r for r in rows}
    res = ReconcileResult()

    for e in evaluations:
        ikey = f"seo_{e.problem_type}:{marketplace or 'unknown'}:{sku or 'unknown'}"
        sig = by_key.get(ikey)

        if e.result == RuleResult.TRIGGERED:
            evh = _evidence_hash(e.evidence)
            ...
            if sig is None:
                new = _new_signal(audit_id=audit_id, user_id=user_id, listing_id=listing_id,
                                  ...
                                  evidence_hash=evh, problem_id=pid, now=now)
                db.add(new); by_key[ikey] = new; res.created += 1
            elif sig.status == RESOLVED:
                _apply_draft(sig, draft, evidence_hash=evh, audit_id=audit_id,
                             problem_id=pid, now=now, status=REOPENED); res.reopened += 1
                ...
                if sig.evidence_hash != evh:
                    _apply_draft(sig, draft, evidence_hash=evh, audit_id=audit_id,
                                 problem_id=pid, now=now, status=sig.status); res.updated += 1
                else:
                    res.unchanged += 1
            else:  # promoted_to_decision — Decision owns its life, leave it
                res.unchanged += 1
