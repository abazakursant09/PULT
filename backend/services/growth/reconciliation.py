"""
Growth Signal Reconciliation (A6) — give growth signals memory + lifecycle.
...
canonical insight_key `growth_<problem_type>:<marketplace>:<sku>`. One signal row
per insight_key — a duplicate live signal is structurally impossible.
...
mean the opportunity disappeared.

Pure DB reconciliation: no API, no Decision bridge, no measurement, no AI, no
forecast, no marketplace-specific code. Flush-only.
"""
...
def _apply_draft(sig: GrowthSignal, draft, *, evh, audit_id, problem_id, now, status):
...
def _new_signal(*, audit_id, user_id, listing_id, marketplace, sku, draft, evh, problem_id, now):
    return GrowthSignal(
        ...
        insight_key=draft.insight_key, problem_type=draft.problem_type, category=draft.category,
...
async def reconcile_signals(
    ...
    rows = (await db.execute(select(GrowthSignal).where(
        GrowthSignal.user_id == user_id, GrowthSignal.listing_id == listing_id))).scalars().all()
    by_key = {r.insight_key: r for r in rows}
    res = ReconcileResult()

    for e in evaluations:
        ikey = f"growth_{e.problem_type}:{marketplace or 'unknown'}:{sku or 'unknown'}"
        sig = by_key.get(ikey)
        ...
        if e.result == RuleResult.TRIGGERED:
            ...
            if sig is None:
                new = _new_signal(audit_id=audit_id, user_id=user_id, listing_id=listing_id,
                                  marketplace=marketplace, sku=sku, draft=draft, evh=evh,
                                  problem_id=pid, now=now)
        ...
        else:  # NOT_EVALUATED — unknown, never resolve/touch (opportunity may still exist)
            if sig is not None:
                res.unchanged += 1
