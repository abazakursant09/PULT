  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: '#09090B' }}>
        <RefreshCw size={28} className="animate-spin" style={{ color: '#71717A' }} />
      </div>
    )
  }

  return (
    <div className="min-h-screen" style={{ background: '#09090B', position: 'relative' }}>
      ...
      <main className="max-w-[720px] mx-auto px-5 sm:px-8 py-10 sm:py-14 animate-fade-in" style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div className="mb-10">
          <button onClick={() => router.push('/dashboard/account?tab=security')}
                  ...
                  style={{ color: '#71717A', border: '1px solid rgba(255,255,255,0.08)', background: 'none', cursor: 'pointer' }}>
            <ArrowLeft size={13} /> Назад
          </button>
          <p className="text-xs font-medium mb-2.5 uppercase tracking-wide" style={{ color: '#7C3AED' }}>Настройки</p>
        ...
        {success && (
          <div className="mb-6 px-4 py-3 rounded-xl flex items-center gap-3 animate-slide-up"
               style={{ background: 'rgba(110,106,252,0.10)', border: '1px solid rgba(110,106,252,0.25)', color: '#7C3AED' }}>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-3 shrink-0">
              <Badge variant={mfaEnabled ? 'outline' : 'secondary'}
                     style={mfaEnabled ? { background: 'rgba(110,106,252,0.12)', color: '#7C3AED', borderColor: 'rgba(110,106,252,0.28)' } : {}}>
        ...
        {phase === 'setup' && (
          <Card className="p-7 sm:p-9 animate-slide-up mb-6">
            ...
            <div className="mb-5">
              <Label className="mb-2 block" style={{ color: '#71717A' }}>Секретный ключ (ввести вручную)</Label>
              <div className="flex items-center gap-3 px-4 py-3 rounded-xl"
                   style={{ background: 'rgba(110,106,252,0.06)', border: '1px solid rgba(110,106,252,0.18)' }}>
                <code className="flex-1 text-sm font-semibold tracking-widest break-all font-mono" style={{ color: '#7C3AED' }}>
                  {secret}
                </code>
                <Button variant="ghost" size="icon" onClick={copySecret} title="Скопировать">
                  {copied ? <Check size={14} style={{ color: '#7C3AED' }} /> : <Copy size={14} />}
                </Button>
              </div>
            </div>

            <div className="mb-8">
              <a href={otpauth} className="inline-flex items-center gap-2 text-sm font-medium" style={{ color: '#7C3AED' }}>
            ...
            <div className="mb-8 rounded-xl px-5 py-4 space-y-2" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
              {[
                ...
                <div key={i} className="flex items-start gap-3">
                  ...
                  <p className="text-sm" style={{ color: '#71717A' }}>{s}</p>
                </div>
              ))}
            </div>

            <div className="mb-5 space-y-2">
              <Label htmlFor="totp-setup" style={{ color: '#71717A' }}>Код подтверждения (6 цифр)</Label>
              <Input
                id="totp-setup"
                type="text"
                inputMode="numeric"
        ...
        {phase === 'disable' && (
          <Card className="p-7 sm:p-9 animate-slide-up mb-6">
            ...
            <div className="mb-5 space-y-2">
              <Label htmlFor="totp-disable" style={{ color: '#71717A' }}>Код аутентификатора</Label>
              <Input
                id="totp-disable"
                type="text"
                inputMode="numeric"
          ...
          <div className="rounded-2xl px-6 py-5 mt-4" style={{ background: 'rgba(110,106,252,0.06)', border: '1px solid rgba(110,106,252,0.14)' }}>
            ...
            <ul className="space-y-1.5">
              {[
                ...
                <li key={i} className="flex items-center gap-2 text-sm" style={{ color: '#71717A' }}>
