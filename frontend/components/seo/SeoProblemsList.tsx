'use client'
import type { SeoProblem } from '@/lib/api'

/** SeoProblemsList — найденные проблемы карточки последнего аудита. */
...
export function SeoProblemsList({ problems }: { problems: SeoProblem[] }) {
  if (!problems.length) {
    return (
      <div style={{ fontSize: 12.5, color: 'var(--text-3)', textAlign: 'center', padding: '12px 0' }}>
        Проблем по карточке не найдено.
      </div>
    )
  }
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 12, padding: '2px 14px' }}>
      {problems.map((p, i) => (
        <div key={p.problem_type} style={{
          display: 'flex', alignItems: 'flex-start', gap: 12, padding: '11px 0',
          borderBottom: i < problems.length - 1 ? '1px solid var(--line)' : 'none',
        }}>
          <span style={{ fontSize: 10, fontWeight: 700, color: sevColor(p.severity), flex: '0 0 auto', minWidth: 64 }}>
...
export default SeoProblemsList
