'use client'
import type { LegalAuditItem } from '@/lib/api'
...
export function LegalAuditHistory({ audits }: { audits: LegalAuditItem[] }) {
  if (!audits.length) {
    return (
      <div style={{ fontSize: 12.5, color: 'var(--text-3)', textAlign: 'center', padding: '14px 0' }}>
        Проверок юридических рисков ещё не было.
      </div>
    )
  }
  return (
        ...
        return (
          <div key={a.audit_id} style={{
            ...
            <div style={{ display: 'flex', gap: 14, fontSize: 11.5, color: 'var(--text-2)' }}>
              <span>Отмечено: <b>{a.total_findings}</b></span>
...
export default LegalAuditHistory
