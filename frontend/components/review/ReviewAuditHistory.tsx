'use client'
import type { ReviewAuditItem } from '@/lib/api'
...
export function ReviewAuditHistory({ audits }: { audits: ReviewAuditItem[] }) {
  if (!audits.length) {
    return (
      <div style={{ fontSize: 12.5, color: 'var(--text-3)', textAlign: 'center', padding: '14px 0' }}>
        Проверок отзывов ещё не было.
      </div>
    )
  }
  return (
        ...
        return (
          <div key={a.audit_id} style={{
            ...
            <div style={{ display: 'flex', gap: 14, fontSize: 11.5, color: 'var(--text-2)' }}>
              <span>Проблем: <b>{a.total_problems}</b></span>
              <span>Не оценено: {a.total_not_evaluated}</span>
              {a.top_severity && (
                <span style={{ color: a.top_severity === 'critical' ? 'var(--danger)' : 'var(--text-2)' }}>
                  {SEVERITY_RU[a.top_severity] ?? a.top_severity}
                </span>
              )}
            </div>
          </div>
...
export default ReviewAuditHistory
